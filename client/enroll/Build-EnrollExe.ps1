# Build a single signed Enroll-BitLocker.exe from Enroll-BitLocker.ps1 (PS2EXE).
# One file, double-click, AppLocker-friendly (signed). Requires ps2exe module.
#   .\Build-EnrollExe.ps1 -Pfx "C:\path	o\CodeSign.pfx" -Password "***"
param(
    [string]$Pfx,
    [string]$Password,
    [string]$Thumbprint
...
$src = Join-Path $dir "Enroll-BitLocker.ps1"
$out = Join-Path $dir "Enroll-BitLocker.exe"

if (-not (Get-Module -ListAvailable -Name ps2exe)) {
    Write-Host "Installing ps2exe..."; Install-Module ps2exe -Scope CurrentUser -Force
}
Import-Module ps2exe
Write-Host "Compiling -> Enroll-BitLocker.exe"
Invoke-ps2exe -inputFile $src -outputFile $out -requireAdmin `
    -title "BitLocker Enrollment" -product "BitLocker Escrow Enroll" -company "YourOrg" -version "1.0.0.0" -noConfigFile
